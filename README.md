...
- `start_time` (整数): 查询起始时间（Unix 秒）。
- `end_time` (整数): 查询结束时间（Unix 秒，默认当前时间）。
- `user_ids` (整数数组): 仅返回指定 QQ 号发送的消息。
- `keywords` (字符串数组): 关键词（OR 逻辑）匹配正文、发送者昵称、发送者 QQ 号。
- `limit` (整数): 最多返回消息数量（1-500）。
- `hours` (数字): 兼容旧参数，仅在未提供 `start_time` 时生效。
- `count` (整数): 兼容旧参数，仅在未提供 `limit` 时生效。
